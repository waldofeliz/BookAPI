...
public sealed class CreateAutorHandler : IRequestHandler<CreateAutorCommand, AutorDto>
{
    ...
    public async Task<AutorDto> Handle(CreateAutorCommand request, CancellationToken ct)
    {
        ...
        var autor = new Autor(request.Nombre, request.Apellido, request.Biografia, request.Cumpleanio, request.CreadoPor, request.Nacionalidad, request.FechaFallecimiento, request.FotoUrl, request.SitioWeb);
